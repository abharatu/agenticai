...
const encodedCredentials = node_buffer_1.Buffer.from(`${jiraEmail}:${jiraApiToken}`).toString('base64');
// ----------------------
 ...
 */
function processIssue(issueJSON, isSub = false) {
    ...
    const sprints = Array.isArray(issueJSON.fields.customfield_10016)
        ? issueJSON.fields.customfield_10016.map(e => `${e.name} (id:${e.id})`).join(',')
        : '';
    ...
    const comments = Array.isArray(issueJSON.fields.comment?.comments)
        ...
        : [];
    const processedData = { id, status, statusCatatory, description, parent, parentLink, assignee, reporter, creator, created, issuetype, project, sprints, summary, updated, comments };
 ...
 */
async function fetchJiraAPI(url, method = 'GET', body = null) {
 ...
 */
async function getIssue(ticket) {
 ...
 */
async function getActiveSprint(boardId = 1197) {
...
async function getSprintIssues(sprintId) {
    return await fetchJiraAPI(`/rest/agile/1.0/sprint/${sprintId}/issue`).then(data => {
        if (!data.issues || data.issues.length === 0)
            return "No issues found for this sprint.";
        let md = `\n# Issues for Sprint ID: ${sprintId}\n`;
        md += data.issues.map((issue) => {
 ...
 * @returns Markdown string with issues summary
 */
async function getActiveSprintIssues(boardId = 1197) {
        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
            try {
                if (request.params.name === 'jira_issue') {
                    ...
                    const args = request.params.arguments;
                    if (!args.sprintId) {
                        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Missing required parameter: sprintId');
                    }
                ...
                if (error instanceof Error) {
                    ...
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Jira search failed: ${error.message}`);
                }
                throw error;
            }
        });
